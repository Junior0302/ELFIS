...
export default function EmailConnectionsPanel({ token, orgId, canManage, userEmail }: Props) {
  ...
  return (
    <section className="email-connections-panel" aria-label="Boîte mail d’expédition">
      ...
      {canManage && !googleOk && !microsoftOk && (
        <p className="form-error" role="status">
          Google / Microsoft ne sont pas encore configurés côté serveur (variables OAuth Render).
          Vous pouvez déjà utiliser <strong>Configurer un autre fournisseur</strong> (SMTP de votre
          boîte : Gmail avec mot de passe d’application, Outlook, OVH…).
        </p>
      )}
      ...
      {canManage && (
        <div className="actions" style={{ flexWrap: 'wrap', marginBottom: '1rem' }}>
          <button
            ...
            title={
              platformOk
                ? 'Envoi via Brevo (nécessite une clé Brevo valide)'
                : 'Infrastructure ComptaPilot non configurée'
            }
          >
          ...
          <button
            ...
            title={googleOk ? undefined : 'GOOGLE_CLIENT_* non configuré sur Render'}
          >
            Connecter Google
          </button>
          <button
            ...
            title={microsoftOk ? undefined : 'MICROSOFT_CLIENT_* non configuré sur Render'}
          >
            Connecter Microsoft
          </button>
          <button
          ...
          >
            Configurer un autre fournisseur (SMTP)
          </button>
        </div>
      )}
