"""Certification PostgreSQL Smart Migration Sprint 7."""

from __future__ import annotations

import json
import os
import sys
from datetime import datetime, timezone
from pathlib import Path

BACKEND = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(BACKEND))

from scripts.migration.certify_document_extraction_sprint4_postgres import (  # noqa: E402
    apply_sql_whole,
    ensure_base,
    _engine,
    _q,
)

SPRINT6 = BACKEND / "sql" / "elfis_import_engine_sprint6_postgres.sql"
SPRINT5 = BACKEND / "sql" / "elfis_validation_mapping_sprint5_postgres.sql"
SPRINT4 = BACKEND / "sql" / "elfis_document_extraction_sprint4_postgres.sql"
SPRINT3 = BACKEND / "sql" / "elfis_document_analysis_sprint3_postgres.sql"
SPRINT7 = BACKEND / "sql" / "elfis_smart_migration_sprint7_postgres.sql"
OUT = BACKEND / "docs" / "migration" / "sprint7-postgres-certification.json"


def verify(eng) -> dict:
    tables = [
        r[0]
        for r in _q(
            eng,
            "SELECT table_name FROM information_schema.tables WHERE table_schema='public' "
            "AND table_name LIKE 'elfis_smart_migration_%' ORDER BY 1",
        )
    ]
    needed = {
        "elfis_smart_migration_runs",
        "elfis_smart_migration_batches",
        "elfis_smart_migration_batch_items",
        "elfis_smart_migration_reports",
        "elfis_smart_migration_cleanup_log",
    }
    missing = sorted(needed - set(tables))
    idxs = [
        r[0]
        for r in _q(
            eng,
            "SELECT indexname FROM pg_indexes WHERE schemaname='public' "
            "AND tablename LIKE 'elfis_smart_migration_%' ORDER BY 1",
        )
    ]
    return {
        "ok": not missing and len(idxs) >= 5,
        "tables": tables,
        "missing": missing,
        "indexes": idxs,
    }


def main() -> int:
    eng = _engine()
    ensure_base(eng)
    if not _q(
        eng,
        "SELECT 1 FROM information_schema.tables WHERE table_name='elfis_import_runs'",
    ):
        apply_sql_whole(eng, SPRINT3)
        apply_sql_whole(eng, SPRINT4)
        apply_sql_whole(eng, SPRINT5)
        apply_sql_whole(eng, SPRINT6)

    results = []
    apply_sql_whole(eng, SPRINT7)
    results.append({"scenario": "A_apply", **verify(eng)})
    apply_sql_whole(eng, SPRINT7)
    results.append({"scenario": "B_idempotent", **verify(eng)})

    # contraintes uniques
    uq = _q(
        eng,
        "SELECT conname FROM pg_constraint WHERE conname IN "
        "('uq_elfis_sm_batch_idx','uq_elfis_sm_item_doc','uq_elfis_sm_rep_ver')",
    )
    results.append(
        {"scenario": "C_constraints", "ok": len(uq) >= 3, "constraints": [r[0] for r in uq]}
    )

    all_ok = all(r["ok"] for r in results)
    report = {
        "certified": all_ok,
        "sprint": "7",
        "scenarios": results,
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "environment": os.getenv("ELFIS_ENVIRONMENT"),
    }
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(report, indent=2, default=str), encoding="utf-8")
    print(json.dumps(report, indent=2, default=str))
    return 0 if all_ok else 1


if __name__ == "__main__":
    raise SystemExit(main())
